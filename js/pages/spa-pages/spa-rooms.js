...
import { isSuperAdmin, getUser } from '../../auth/session.js';
...
async function enrichRoomsWithGuests(filterCamp) {
...
export async function init() {
  ...
  const filterCamp = document.getElementById('rm-filter-camp');
  const filterBlock = document.getElementById('rm-filter-block');
  const searchInput = document.getElementById('rm-filter-search');
  ...
  const formCampSelect = document.getElementById('room-campId');
  const formBlockSelect = document.getElementById('room-blockId');
  ...
  if (canManage) {
    ...
    form?.addEventListener('submit', (e) => onSave(e, form, submitBtn, filterCamp, filterBlock, formCampSelect, formBlockSelect));
  } else if (addBtn) {
    addBtn.hidden = true;
  }

  tableBody.addEventListener('click', (e) => onTableClick(e, tableBody, titleEl, filterCamp, filterBlock, formCampSelect, formBlockSelect));
  filterCamp.addEventListener('change', () => loadBlocksForFilter(filterCamp, filterBlock));
  filterBlock.addEventListener('change', () => loadRooms(filterCamp, filterBlock, tableBody));
  searchInput?.addEventListener('input', () => {
    searchQuery = searchInput.value.trim().toLowerCase();
    renderTable(tableBody);
  });

  formCampSelect?.addEventListener('change', () => loadBlocksForForm(formCampSelect, formBlockSelect));
  loadCamps(filterCamp, filterBlock, tableBody, formCampSelect);
}
...
export async function refresh() {
  ...
  const filterBlock = document.getElementById('rm-filter-block');
  const tableBody = document.getElementById('rooms-table-body');
  loadRooms(filterCamp, filterBlock, tableBody);
}

async function loadCamps(filterCamp, filterBlock, tableBody, formCampSelect) {
  try {
...
async function loadBlocksForFilter(filterCamp, filterBlock) {
...
async function loadBlocksForForm(formCampSelect, formBlockSelect, selectedBlockId = '') {
  formBlockSelect.innerHTML = '<option value="">Select camp first</option>';
  formBlockSelect.disabled = !formCampSelect.value;
  if (!formCampSelect.value) return;

  const response = await listBlocks(formCampSelect.value, { includeInactive: 'true' });
  const items = response.data?.blocks || response.data?.items || response.data || [];
  fillSelect(
    formBlockSelect,
...
function loadRooms(filterCamp, filterBlock, tableBody) {
  window.clearTimeout(loadRoomsTimer);
  loadRoomsTimer = window.setTimeout(() => {
    runLoadRooms(filterCamp, filterBlock, tableBody);
  }, 250);
}

async function runLoadRooms(filterCamp, filterBlock, tableBody) {
  ...
  try {
    ...
    await enrichRoomsWithGuests(filterCamp);
    if (seq !== loadRoomsSeq) return;
    renderTable(tableBody);
  } catch (error) {
    if (seq !== loadRoomsSeq) return;
...
async function openRoomModal(titleEl, filterCamp, filterBlock, formCampSelect, formBlockSelect, room = null) {
  const form = document.getElementById('room-form');
  ...
  if (room) {
    ...
    await loadBlocksForForm(formCampSelect, formBlockSelect, blockId);
    ...
    if (filterCamp.value) {
      ...
      await loadBlocksForForm(formCampSelect, formBlockSelect, filterBlock.value);
    }
  }
  openModal('room');
}

async function onSave(event, form, submitBtn, filterCamp, filterBlock, formCampSelect, formBlockSelect) {
...
async function toggleMaintenance(room, id, filterCamp, filterBlock, tableBody) {
...
async function onTableClick(event, tableBody, titleEl, filterCamp, filterBlock, formCampSelect, formBlockSelect) {
  ...
  if (action === 'edit' && room) {
    await openRoomModal(titleEl, filterCamp, filterBlock, formCampSelect, formBlockSelect, room);
    return;
  }

  if (action === 'maintenance' || action === 'mark-available') {
    if (room) await toggleMaintenance(room, id, filterCamp, filterBlock, tableBody);
    return;
  }
