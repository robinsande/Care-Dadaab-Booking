...
export async function init() {
  ...
  const filterCamp = document.getElementById('blk-filter-camp');
  const addBtn = document.getElementById('add-block-btn');
  const submitBtn = document.getElementById('block-submit');
  const titleEl = document.getElementById('block-modal-title');
  const formCampSelect = document.getElementById('block-campId');

  addBtn.addEventListener('click', () => openBlockModal(titleEl, filterCamp, formCampSelect));
  form.addEventListener('submit', (e) => onSave(e, form, submitBtn, tableBody, filterCamp));
  tableBody.addEventListener('click', (e) => onTableClick(e, tableBody, titleEl, filterCamp, formCampSelect));
  filterCamp.addEventListener('change', () => loadBlocks(filterCamp, tableBody));
  loadCamps(filterCamp, formCampSelect, tableBody);
}

export async function refresh() {
  const filterCamp = document.getElementById('blk-filter-camp');
  const tableBody = document.getElementById('blocks-table-body');
  loadBlocks(filterCamp, tableBody);
}

async function loadCamps(filterCamp, formCampSelect, tableBody) {
  try {
...
async function loadBlocks(filterCamp, tableBody) {
  ...
  try {
    ...
    renderTable(tableBody, filterCamp);
  } catch (error) {
...
function renderTable(tableBody, filterCamp) {
  if (!blocks.length) {
...
function openBlockModal(titleEl, filterCamp, formCampSelect, block = null) {
  const form = document.getElementById('block-form');
...
async function onSave(event, form, submitBtn, tableBody, filterCamp) {
...
async function onTableClick(event, tableBody, titleEl, filterCamp, formCampSelect) {
  ...
  if (action === 'edit' && block) {
    openBlockModal(titleEl, filterCamp, formCampSelect, block);
    return;
  }
